...
It looks like the year `r dallas_home_sales %>% filter(total_sales == max(total_sales)) %>% pull(year)` had the greatest number of sales. What interesting findings! Let's create a CSV for distribution (this code chunk that generates the CSV is ultimately not shown because we used `include=FALSE` as a chunk option).

```{r write_csv, include=FALSE}
dallas_home_sales %>% write_csv("dallas_home_sales.csv")
```

We can create an email using **RStudio Connect**, one that aligns with the content from this report. We do this with the `render_email()` and `connect_email()` functions from the **blastula** package. We can use a subdocument (included here as `connect-example-email.Rmd`) to craft the contents of the email, drawing upon results available in this document. Attachments can be managed by using **blastula**'s attachment helper functions, containing them in a list.

```{r include=FALSE}
render_email(input = "connect-example-email.Rmd") %>%
  connect_email(
    subject = "RStudio Connect Example Email",
    attachments = list(
      report_rmd(),
      generated_files("dallas_home_sales.csv"),
      connect_files("austin_home_sales.csv")
    )
  )
```
