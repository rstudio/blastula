...
## Using the `glue()` Function for more Sophisticated Email Text

A good strategy for longer and more complicated text inputs is to use the [**glue**](https://glue.tidyverse.org) package. We can continue lines with the `\\` line continuation marker, which is great for keeping long paragraphs of text at a reasonable width. Also, **glue** allows us to use string interpolation (and avoid the use of `paste()`). Variables in the global workspace can be included in the curly braces of a `glue()` string expression, and, any **R** expressions in `{ }` will be evaluated.
...
Markdown text can be used as input for the `body`, `header`, and `footer` arguments. To opt into using Markdown formatting, call the `md()` text helper function on any of these text elements. We can use the usual Markdown-formatting you know and you'll get italicized text, bold text, lists, links, etc. Have a look at [this reference page](https://commonmark.org/help/) to quickly understand how to use Markdown. The **blastula** package is internally using the **commonmark** **R** package to perform the conversion from Markdown to HTML.

A few notes on code style and options. It's good to left-align the text in the code example since it will reduce the risk of having four or more spaces after a linebreak, which will result in `text like this` when using Markdown formatting through `md()`. 
