...
The **blastula** package uses a binary for `smtp` mailing that's provided by the **mailsend-go** project. This binary is cross-platform and works on **Windows**, **macOS** (via **Homebrew**), and **Linux** (**Debian** and **RPM** packages are available). Here are some basic instructions for getting the `mailsend-go` binary installed on your system.
